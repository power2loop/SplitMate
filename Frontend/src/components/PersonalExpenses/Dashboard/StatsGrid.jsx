...
const StatsGrid = ({ expenses }) => {
    ...
    const sameMonthLastYearTotal = sameMonthLastYearExpenses.reduce((total, expense) => total + expense.amount, 0);

    // Calculate Investment Total (ALL Investment category expenses)
    const investmentExpenses = expenses.filter(expense =>
    ...
    const lastMonthInvestments = expenses.filter(expense =>
        isDateInPeriod(expense.date, lastMonth, lastMonthYear) &&
        expense.category === "Investment"
    ...
    const monthDifferenceText = monthDifference === 0 ? "--" :
        monthDifference > 0 ? `+₹${Math.abs(monthDifference).toFixed(2)}` :
                    ...
                    <div className="card-info">
                        ...
                        <div className="investment-progress">
                            ...
                            <span className="progress-text">
                                {parseFloat(investmentGrowthPercent) > 0 ? '+' : parseFloat(investmentGrowthPercent) < 0 ? '' : ''}{investmentGrowthPercent}%
                            </span>
                        </div>
                    </div>
                </div>
