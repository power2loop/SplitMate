...
export default function AddExpenseModal({
  ...
  const handleSubmit = async (e) => {
    ...
    if (method === "equal") {
      ...
      if (normMembers[0]) {
        allocations[normMembers[0].id] =
          Math.round((allocations[normMembers[0].id] + diff) * 100) / 100;
      }
    ...
    try {
      const created =
        mode === "group"
          ? await api(`/expenses/group/${encodeURIComponent(groupId)}`, {
              method: "POST",
              body: groupPayload,
            })
          ...
          <div className="expense-body">
            ...
            {method === "custom" && (
              <div className="split-list">
                ...
                <div
                  className={`sum-row ${Math.round(customSum * 100) !== Math.round(amtNum * 100) ? "err" : ""}`}
                >
